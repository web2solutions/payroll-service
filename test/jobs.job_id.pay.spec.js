...
const { client1, client2, contractor6, client4, contractor5, contractor8, contractor7, contract9, contract1 } = require('./data');
...
describe('/jobs/:job_id/pay suite', () => {
    ...
    it('can not pay for non exiting job', async() => {
        const response = await request(app)
            .get(`/jobs/0/pay`)
    ...
    it('can not pay for a non active contract', async() => {
        ...
        const response = await request(app)
            .get(`/jobs/1/pay`)
            .set({ 'profile_id': 5, Accept: 'application/json' });
